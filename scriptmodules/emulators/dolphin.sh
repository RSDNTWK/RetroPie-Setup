...
function build_dolphin() {
    mkdir build
    cd build
    cmake .. -DENABLE_EGL=ON -DENABLE_EVDEV=ON -DLINUX_LOCAL_DEV=ON -DOpenGL_GL_PREFERENCE=GLVND -DENABLE_TESTS=OFF -DENABLE_LLVM=OFF -DENABLE_ANALYTICS=OFF -DENABLE_X11=OFF -DENCODE_FRAMEDUMPS=OFF -DCMAKE_INSTALL_PREFIX="$md_inst"
    make clean
    make
