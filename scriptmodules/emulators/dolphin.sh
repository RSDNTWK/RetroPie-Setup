...
function build_dolphin() {
    mkdir build
    cd build
    cmake .. -DENABLE_HEADLESS=ON -DENABLE_EGL=ON -DENABLE_EVDEV=ON -DLINUX_LOCAL_DEV=ON -DOpenGL_GL_PREFERENCE=GLVND -DENABLE_TESTS=OFF -DENABLE_LLVM=OFF -DENABLE_ANALYTICS=OFF -DENABLE_X11=OFF -DENCODE_FRAMEDUMPS=OFF -DHAVE_VULKAN=1 -DCMAKE_INSTALL_PREFIX="$md_inst"
    make clean
    make
...
_EOF_
    ...
    addEmulator 1 "$md_id" "gc" "$md_inst/bin/dolphin-emu-nogui -p drm -v Vulkan -e %ROM%"
    addEmulator 0 "$md_id-gui" "gc" "$md_inst/bin/dolphin-emu -b -e %ROM%"
    addEmulator 1 "$md_id" "wii" "$md_inst/bin/dolphin-emu-nogui -p drm -v Vulkan -e %ROM%"
